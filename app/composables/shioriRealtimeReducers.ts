import type { ShioriWithRole } from '~~/types/database'
import type { ShioriRealtimePayload, DayRealtimePayload, EventRealtimePayload } from './realtimeSync'
 ...
 *
 * payload 型は realtimeSync.ts が単一のソースなので、ここでは import のみで再定義しない。
 */
 ...
 */
export function applyShioriChange(old: ShioriWithRole, payload: ShioriRealtimePayload): ShioriWithRole {
  return {
    ...old,
 ...
 */
export function applyDayChange(old: ShioriWithRole, payload: DayRealtimePayload): ShioriWithRole {
  const days = [...old.days]
  ...
  if (payload.eventType === 'UPDATE') {
    ...
    if (idx < 0) {
      // 対象 day が無ければ並び替え不要（無意味な sort を避ける）
      return { ...old, days }
    }
 ...
 */
export function applyEventChange(old: ShioriWithRole, payload: EventRealtimePayload): ShioriWithRole {
